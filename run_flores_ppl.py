...
def evaluate_flores(model_path, lang="sw", split="devtest",
    ...
    ds = load_dataset("facebook/flores", lang, split=split, trust_remote_code=True)
    texts = ds["sentence"]
...
def parse_args():
    ...
    p.add_argument("--out", type=str, default=None, help="Optional JSON path for the exact PPL result.")
    return p.parse_args()


if __name__ == "__main__":
    args = parse_args()
    ppl = evaluate_flores(args.model_path, lang=args.lang,
            ...
            json.dump(
                {
                    ...
                    "fp16": args.fp16,
                    "perplexity": ppl,
                    "formulation": "sentence_level_flores_ppl",
                },
                f,
                indent=2,
            )
