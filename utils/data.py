...
def load_flores_pair(src_code: str, tgt_code: str, split: str = "devtest", dataset: str = "facebook/flores"):
    ...
    # FLORES ships as a custom dataset script, so trust_remote_code must be
    # passed explicitly here; model trust flags do not affect datasets.load_dataset.
    ds_src = load_dataset(dataset, src_code, split=split, trust_remote_code=True)
    ds_tgt = load_dataset(dataset, tgt_code, split=split, trust_remote_code=True)
