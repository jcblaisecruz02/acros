...
def blocking_output_entries(path):
    """Files that should make a fresh run refuse to reuse an output dir."""
    if not os.path.isdir(path):
        return []
    return [name for name in os.listdir(path) if name != "training_args.json"]
...
def get_args(logger):
    ...
    p.add_argument(
        ...
        default="fp16",
        help="Autocast dtype to use when --use_fp16 is enabled. bf16 is safer on A100/H100 because it has fp32-like exponent range.",
    )
    p.add_argument(
        "--train_all_parameters",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Unfreeze all model parameters for adaptation; for SenseLlama also disables config-level backbone/lm-head freezing.",
    )
    p.add_argument(
        "--compile_model",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Opt into torch.compile for adaptation. Disabled by default because SENSiA uses multiple forwards whose outputs are consumed later.",
    )
    p.add_argument(
        "--compile_mode",
        choices=["default", "reduce-overhead", "max-autotune"],
        default="default",
        help="torch.compile mode to use when --compile_model is active.",
    )
    ...
    # Make save_dir and save the config for reproducibility. Under torchrun,
    # all ranks call get_args() before distributed init; rank 0 owns filesystem
    # safety checks to avoid races where one rank creates training_args.json and
    # another rank immediately treats the directory as stale output.
    if is_rank0_process():
        if os.path.exists(args.save_dir):
            if args.overwrite_output_dir:
                ...
                entries = blocking_output_entries(args.save_dir)
                if entries:
                    raise ValueError(
