...
if __name__ == "__main__":
    ...
    if args.train_all_parameters:
        for attr in ("freeze_backbone", "freeze_lm_head"):
    ...
    if args.train_all_parameters:
        ...
        for attr in ("freeze_backbone", "freeze_lm_head"):
            if hasattr(torch_model.config, attr):
                setattr(torch_model.config, attr, False)
        logger.info("[Model] --train_all_parameters active; all model parameters are trainable.")
    ...
    # torch.compile reduce-overhead uses CUDAGraphs, which can overwrite model
    # outputs when the SENSiA loop runs source and target forwards before
    # consuming both outputs. Keep compilation opt-in for adaptation.
    if getattr(args, "compile_model", False):
        try:
            compile_mode = getattr(args, "compile_mode", "default")
            if compile_mode == "default":
                model = torch.compile(torch_model)
            else:
                model = torch.compile(torch_model, mode=compile_mode)
            logger.info(f"torch.compile applied (mode={compile_mode}).")
        except Exception:
            logger.info("torch.compile not applied (unsupported graph).")
    else:
        logger.info("torch.compile disabled for adaptation.")
    ...
    module_for_breakdown = getattr(torch_model, "backpack", torch_model)
    log_param_breakdown(module_for_breakdown, logger)
