"""
modeling_sense_opt.py -- Residual sense-induction language model for OPT.

This model preserves a frozen OPT LM path and adds a gated Backpack-style
sense path:
...
try:
    from .configuration_sense_opt import SenseOPTConfig
...
@dataclass
class SenseOPTLMHeadModelOutput(ModelOutput):
...
class SenseOPTLMHeadModel(OPTPreTrainedModel, GenerationMixin):
    config_class = SenseOPTConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = False
    _no_split_modules = ["OPTDecoderLayer", "SenseNoMixBlock"]
    accepts_loss_kwargs = False
    _tied_weights_keys = {
        "lm_head.weight": "model.decoder.embed_tokens.weight",
        "word_embeddings.weight": "model.decoder.embed_tokens.weight",
    }
    _keys_to_ignore_on_load_missing = [r"lm_head\.weight", r"word_embeddings\.weight"]

    def __init__(self, config: SenseOPTConfig, opt_model: Optional[OPTModel] = None):
        super().__init__(config)
        self.model = opt_model if opt_model is not None else OPTModel(config)
        self.word_embeddings = self.model.decoder.embed_tokens
        self.num_senses = config.num_senses
        self.embed_dim = int(getattr(config, "word_embed_proj_dim", config.hidden_size))
        self.min_train_gate = 0.0

        dropout = getattr(config, "dropout", 0.0)
        act_fn = ACT2FN[getattr(config, "activation_function", "relu")]
        ...
        self.aux_recon_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=False)
    ...
    def from_base_lm(
        ...
        if not hasattr(base_lm, "model"):
            raise ValueError("SenseOPTLMHeadModel.from_base_lm expects an OPTForCausalLM-style `.model` backbone.")
        ...
        model = cls(cfg, opt_model=base_lm.model)
    ...
    def forward(
        ...
        **kwargs,
        ...
        opt_out = self._backbone_forward(
            input_ids=input_ids,
            attention_mask=attention_mask,
        ...
        base_hidden_states = opt_out.last_hidden_state
        ...
        output = SenseOPTLMHeadModelOutput(
            logits=lm_logits,
            ...
            past_key_values=opt_out.past_key_values,
        )
        if return_dict:
            return output
        return (
